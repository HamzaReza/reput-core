...
export async function POST(req: NextRequest) {
  ...
  if (!firstName || !lastName || !country || !description || !Array.isArray(keywords) || keywords.length === 0) {
    return NextResponse.json(
  ...
  try {
    ...
    const usedKeywords = Array.isArray(keywords) && keywords.length > 0 ? keywords : [];
    console.log("[generate-lead] Keywords from client:", usedKeywords);

    const searchQueries =
      usedKeywords.length > 0
        ? usedKeywords.map((kw) => `${sanitizedName} ${kw}`)
        : [`${sanitizedName}`];
