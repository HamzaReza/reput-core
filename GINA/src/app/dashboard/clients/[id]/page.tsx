...
import { clientsApi, isAdmin, webAnalysts, ClientDetail, ClientEventType, ClientAddEventPayload, WebAnalystItem } from "@/lib/api";
import { useEffect, useRef, useState } from "react";
import { useParams, useRouter } from "next/navigation";

// ── Meta ───────────────────────────────────────────────────────────────────────

const EVENT_META: Record<ClientEventType, { label: string; color: string; dot: string }> = {
  ...
  contract_created:  { label: "Contract Created",  color: "#8b5cf6", dot: "#8b5cf6" },
  meeting_set:       { label: "Meeting Set",       color: "#48D4B8", dot: "#48D4B8" },
};

const RISK_COLORS: Record<string, string> = {
  Good: "#22c55e", Mediocre: "#eab308", Poor: "#f97316", Negative: "#ef4444",
};
...
function summarizeEventData(eventType: ClientEventType, data: Record<string, unknown>): string {
  if (eventType === "research") {
    const summary = data.summary as string | undefined;
    const keywords = data.keywords as string[] | undefined;
    return [summary, keywords?.length ? `Keywords: ${keywords.join(", ")}` : ""].filter(Boolean).join(" | ");
  }
  if (eventType === "scan") {
    const score = data.score as number | undefined;
    const linksCount = data.links_count as number | undefined;
    const negativeCount = data.negative_count as number | undefined;
    const headline = (data.summary as { headline?: string } | undefined)?.headline;
    return [
  ...
  return [data.notes ? String(data.notes) : "", data.reason ? String(data.reason) : "", data.date ? `Date: ${String(data.date)}` : ""]
    .filter(Boolean)
    .join(" | ");
}
...
const labelStyle: React.CSSProperties = {
  ...
  color: "var(--color-muted, #64748b)", marginBottom: "0.375rem",
};
...
function ResearchDetail({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      {summary && <p style={{ fontSize: "0.8125rem", color: "#475569", lineHeight: 1.6, margin: 0 }}>{summary}</p>}
...
function ScanDetail({ data, router }: { data: Record<string, unknown>; router: ReturnType<typeof useRouter> }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      ...
      {summary?.headline && <p style={{ fontSize: "0.8125rem", color: "#475569", fontStyle: "italic", margin: 0 }}>"{summary.headline}"</p>}
      {keywords && keywords.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem" }}>
          {keywords.map((kw) => (
            <span key={kw} style={{ fontSize: "0.75rem", padding: "0.15rem 0.6rem", borderRadius: "999px", backgroundColor: "rgba(68,121,218,0.08)", color: "#4479da", border: "1px solid rgba(68,121,218,0.2)" }}>
...
function QuoteSentDetail({ data }: { data: Record<string, unknown> }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
      {!!data.plan_type && <p style={{ fontSize: "0.8125rem", color: "#475569", margin: 0 }}><strong>Plan:</strong> {String(data.plan_type)}</p>}
      {!!data.amount && <p style={{ fontSize: "0.8125rem", color: "#475569", margin: 0 }}><strong>Amount:</strong> ${String(data.amount)}</p>}
      {!!data.message && <p style={{ fontSize: "0.8125rem", color: "#475569", margin: 0, lineHeight: 1.5 }}>{String(data.message)}</p>}
    </div>
  );
}
...
function NoteDetail({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
      {!!data.date && <p style={{ fontSize: "0.8125rem", color: "#475569", margin: 0 }}><strong>Date:</strong> {fmtDate(String(data.date))}</p>}
      {!!text && <p style={{ fontSize: "0.8125rem", color: "#475569", margin: 0, lineHeight: 1.5 }}>{String(text)}</p>}
    </div>
  );
}
...
function QuoteHistory({ events }: { events: ClientDetail["events"] }) {
  const quoteEvents = events.filter((e) =>
    e.event_type === "quote_sent" || e.event_type === "quote_accepted" || e.event_type === "quote_rejected"
  );
  ...
  return (
    <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "1.25rem", marginBottom: "1.25rem" }}>
      <p style={{ fontSize: "0.875rem", fontWeight: 700, color: "var(--color-foreground, #1e293b)", margin: "0 0 1rem" }}>
        Quote History
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
        {quoteEvents.map((e) => {
          ...
          const data = e.data ?? {};
          return (
            <div key={e.id} style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", padding: "0.75rem 1rem", borderRadius: "0.625rem", backgroundColor: "#f8fafc", border: "1px solid var(--color-border, #e2e8f0)" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", minWidth: 0 }}>
                {e.event_type === "quote_sent" && (
                  <>
                    <p style={{ margin: 0, fontSize: "0.875rem", fontWeight: 600, color: "#1e293b" }}>
                      {data.plan_type ? String(data.plan_type) : "Quote"}{data.amount ? ` · $${data.amount}` : ""}
                    </p>
                    {data.message && <p style={{ margin: 0, fontSize: "0.75rem", color: "#64748b" }}>{String(data.message)}</p>}
                  </>
                )}
                {(e.event_type === "quote_accepted" || e.event_type === "quote_rejected") && (
                  <p style={{ margin: 0, fontSize: "0.875rem", fontWeight: 600, color: meta.color }}>{meta.label}</p>
                )}
                {!!data.notes && e.event_type !== "quote_sent" && (
                  <p style={{ margin: 0, fontSize: "0.75rem", color: "#64748b" }}>{String(data.notes)}</p>
                )}
              </div>
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem", flexShrink: 0 }}>
                <span style={{ fontSize: "0.6875rem", fontWeight: 700, color: meta.color, backgroundColor: meta.color + "18", padding: "0.15rem 0.5rem", borderRadius: "999px" }}>
                  {meta.label}
                </span>
                <span style={{ fontSize: "0.6875rem", color: "#94a3b8" }}>{formatDateTime(e.created_at)}</span>
              </div>
            </div>
          );
        })}
...
function ActionPanel({
  ...
  const resetForm = () => { setFormMode(null); setMessage(""); setAmount(""); setNotes(""); setMeetingDate(""); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    let payload: ClientAddEventPayload;
    if (formMode === "quote") {
      payload = { event_type: "quote_sent", event_data: { plan_type: planType, message, amount: parseFloat(amount) } };
      ...
      payload = { event_type: "contract_created", event_data: { notes, date: meetingDate } };
    } else return;
    ...
    return (
      <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "1.25rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
          ...
          <p style={{ margin: 0, fontSize: "0.9rem", fontWeight: 600, color: "#8b5cf6" }}>Contract created — client onboarded</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "1.25rem" }}>
      <p style={{ fontSize: "0.875rem", fontWeight: 700, color: "var(--color-foreground, #1e293b)", margin: "0 0 0.875rem" }}>
        Next Step
      </p>
          ...
          {formMode === "quote" && (
            <form onSubmit={handleSubmit} style={{ borderTop: "1px solid #e2e8f0", paddingTop: "1.25rem", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              <div>
                ...
                <select required value={planType} onChange={(e) => setPlanType(e.target.value)} style={{ ...inputStyle, cursor: "pointer" }}>
                  {PLAN_TYPES.map((p) => <option key={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label style={labelStyle}>Amount</label>
                <input required type="number" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="e.g. 2500" style={inputStyle} />
              </div>
              <div>
          ...
          {formMode === "contract" && (
            <form onSubmit={handleSubmit} style={{ borderTop: "1px solid #e2e8f0", paddingTop: "1.25rem", display: "flex", flexDirection: "column", gap: "0.875rem" }}>
              <div>
                ...
                <input required type="date" value={meetingDate} onChange={(e) => setMeetingDate(e.target.value)} style={inputStyle} />
              </div>
              <div>
...
export default function ClientDetailPage() {
  ...
  useEffect(() => {
    clientsApi.get(id).then(setClient).catch(() => {}).finally(() => setLoading(false));
  }, [id]);

  useEffect(() => {
    if (adminView) webAnalysts.list().then(setAnalystsList).catch(() => {});
  }, [adminView]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      await handleAddEvent(
        accepted
          ? { event_type: "quote_accepted", event_data: { notes: "Auto-simulated acceptance (dev)" } }
          : { event_type: "quote_rejected", event_data: { reason: "Auto-simulated rejection (dev)" } },
      );
    }, delay);
  ...
  if (loading) {
    return (
      <div style={{ padding: "clamp(1.25rem, 4vw, 2rem)", backgroundColor: "#f8fafc", minHeight: "100%" }}>
        <div style={{ height: 20, width: "30%", backgroundColor: "#f1f5f9", borderRadius: 6, marginBottom: "0.5rem" }} />
        <div style={{ height: 14, width: "20%", backgroundColor: "#f1f5f9", borderRadius: 6 }} />
      </div>
    );
  }
  ...
  if (!client) {
    return (
      <div style={{ padding: "clamp(1.25rem, 4vw, 2rem)", backgroundColor: "#f8fafc", minHeight: "100%" }}>
        <p style={{ color: "var(--color-muted, #64748b)" }}>Client not found.</p>
      </div>
    );
  }

  const lastEventType = client.events[client.events.length - 1]?.event_type as ClientEventType | undefined;
  const handleExportTimelinePdf = () => {
    ...
    const timelineRows = client.events
      .map((event) => {
        const meta = EVENT_META[event.event_type as ClientEventType] ?? { label: event.event_type, color: "#64748b", dot: "#94a3b8" };
        const details = summarizeEventData(event.event_type as ClientEventType, (event.data ?? {}) as Record<string, unknown>);
        return `<div class="row">
          <div class="row-head">
            <span class="label" style="background:${meta.color}18;color:${meta.color};border:1px solid ${meta.color}40;">${escapeHtml(meta.label)}</span>
            <span class="time">${escapeHtml(formatDateTime(event.created_at))}</span>
          </div>
...
<meta charset="utf-8"/>
<title>Client Timeline — ${escapeHtml(client.name)}</title>
<style>
  body {
  ...
  h1 { margin: 0 0 0.25rem; font-size: 1.3rem; letter-spacing: -0.01em; }
  .meta { margin: 0 0 1.1rem; color: #64748b; font-size: 0.86rem; }
  .row {
    border: 1px solid #e2e8f0;
    border-radius: 10px;
    padding: 0.72rem 0.85rem;
    margin-bottom: 0.55rem;
    background: linear-gradient(135deg, rgba(68,121,218,0.04), rgba(255,255,255,0) 55%);
  }
  .row-head { display: flex; justify-content: space-between; gap: 0.75rem; margin-bottom: 0.2rem; }
  .label {
    font-weight: 700;
    font-size: 0.73rem;
    border-radius: 999px;
  ...
  .time { color: #64748b; font-size: 0.74rem; white-space: nowrap; }
  .details { margin: 0; color: #475569; font-size: 0.8rem; line-height: 1.55; }
  .footer { margin-top: 1.3rem; padding-top: 0.7rem; border-top: 1px solid #e2e8f0; color: #94a3b8; font-size: 0.74rem; }
  @media print { body { margin: 20px; } }
</style>
</head><body>
<div class="wrap">
<h1>Client Timeline</h1>
<p class="meta">${escapeHtml(client.name)}${client.company ? ` · ${escapeHtml(client.company)}` : ""}${client.country ? ` · ${escapeHtml(client.country)}` : ""}</p>
${timelineRows || "<p>No timeline events available.</p>"}
<p class="footer">Generated by GINA · ${new Date().toLocaleString("en-US", { month: "long", day: "numeric", year: "numeric", hour: "numeric", minute: "2-digit" })}</p>
</div>
</body></html>`);
    win.document.close();
    win.focus();
  ...
  return (
    <div style={{ padding: "clamp(1.25rem, 4vw, 2rem)", backgroundColor: "#f8fafc", minHeight: "100%", boxSizing: "border-box" }}>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <button onClick={() => router.push("/dashboard/clients")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.375rem", color: "var(--color-muted, #64748b)", fontSize: "0.8125rem", fontWeight: 500, padding: 0, marginBottom: "0.875rem" }}>
      ...
      <div style={{ maxWidth: "42rem" }}>
          ...
          <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "1.25rem", marginBottom: "1.25rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "0.875rem" }}>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "#94a3b8", minWidth: 140 }}>Researched by</span>
                <span style={{ fontSize: "0.8125rem", color: "#1e293b" }}>
                  {client.researched_by_name ?? "—"}
                  {client.researched_by_role && (
                    <span style={{ marginLeft: "0.375rem", fontSize: "0.6875rem", fontWeight: 600, color: "#4479da", backgroundColor: "rgba(68,121,218,0.08)", padding: "0.1rem 0.5rem", borderRadius: "999px", border: "1px solid rgba(68,121,218,0.2)" }}>
                      {client.researched_by_role}
                    </span>
                  )}
                </span>
              ...
              {client.scanned_by_name && (
                <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                  <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "#94a3b8", minWidth: 140 }}>Scanned by</span>
                  <span style={{ fontSize: "0.8125rem", color: "#1e293b" }}>
                    {client.scanned_by_name}
              ...
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "#94a3b8", minWidth: 140 }}>Assigned to</span>
                <span style={{ fontSize: "0.8125rem", color: "#1e293b" }}>
                  {client.assigned_to_name ?? <span style={{ color: "#94a3b8", fontStyle: "italic" }}>Unassigned</span>}
                </span>
              </div>
            </div>
            ...
            {!reassignOpen ? (
              <button
                type="button"
                onClick={() => { setReassignId(client.assigned_to_id ?? ""); setReassignOpen(true); }}
                style={{ padding: "0.4rem 1rem", borderRadius: "999px", fontSize: "0.8125rem", fontWeight: 600, border: "1.5px solid #e2e8f0", backgroundColor: "#fff", color: "#1e293b", cursor: "pointer" }}
              >
              ...
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap" }}>
                <select
                  ...
                  style={{ ...inputStyle, width: "auto", minWidth: 180, flex: 1 }}
                >
                  <option value="">— Unassign —</option>
                  {analystsList.filter((a) => a.role === "analyst").map((a) => (
                ...
                <button
                  ...
                  className="glow-button"
                  style={{ padding: "0.4rem 1rem", fontWeight: 700, borderRadius: "999px", fontSize: "0.8125rem", opacity: reassigning ? 0.6 : 1, cursor: reassigning ? "default" : "pointer" }}
                >
                ...
                <button
                  ...
                  style={{ padding: "0.4rem 0.875rem", borderRadius: "999px", fontSize: "0.8125rem", fontWeight: 600, border: "1.5px solid #e2e8f0", backgroundColor: "#fff", color: "#64748b", cursor: "pointer" }}
                >
                  Cancel
                </button>
              </div>
        ...
        {/* Full timeline */}
        <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "1.5rem" }}>
          <p style={{ fontSize: "0.875rem", fontWeight: 700, color: "var(--color-foreground, #1e293b)", margin: "0 0 1.25rem" }}>
            Timeline
          </p>

          {client.events.length === 0 && (
            <p style={{ fontSize: "0.875rem", color: "var(--color-muted, #64748b)", margin: 0 }}>No events yet.</p>
          )}
              ...
              return (
                <div key={event.id} style={{ display: "flex", gap: "1rem", position: "relative" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                    <div style={{ width: 12, height: 12, borderRadius: "50%", backgroundColor: meta.dot, border: "2px solid #fff", boxShadow: `0 0 0 2px ${meta.dot}40`, flexShrink: 0, marginTop: "0.15rem" }} />
                    {!isLast && <div style={{ width: 2, flex: 1, backgroundColor: "#e2e8f0", minHeight: "1.5rem", margin: "0.25rem 0" }} />}
                  </div>
