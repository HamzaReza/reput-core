...
// ── KeywordsEditor ─────────────────────────────────────────────────────────────
function KeywordsEditor({ keywords, setKeywords }: { keywords: string[]; setKeywords: (kws: string[]) => void }) {
  ...
  return (
    <div
      ...
      onClick={() => (document.getElementById("lead-kw-input") as HTMLInputElement | null)?.focus()}
    >
      {keywords.map((kw) => (
        <span key={kw} style={{ display: "inline-flex", alignItems: "center", gap: "0.375rem", padding: "0.25rem 0.75rem", borderRadius: "0.5rem", backgroundColor: "#4479DA", color: "#fff", fontSize: "0.8125rem", fontWeight: 500, whiteSpace: "nowrap" }}>
          {kw}
          <button type="button" onClick={(e) => { e.stopPropagation(); remove(kw); }}
      ...
      <input
        ...
        onKeyDown={(e) => {
          if ((e.key === "Enter" || e.key === "," || e.key === "Tab") && input.trim()) { e.preventDefault(); add(input); }
          else if (e.key === "Backspace" && !input && keywords.length) setKeywords(keywords.slice(0, -1));
        }}
...
export default function LeadPage() {
  ...
  const handleDescriptionChange = (val: string) => {
    setDescription(val);
    if (keywordsReady) { setKeywordsReady(false); setEditableKeywords([]); }
  };

  const handleGenerateKeywords = async () => {
    if (!firstName.trim() || !lastName.trim() || !country || !description.trim()) {
    ...
    try {
      const res = await fetch("/api/generate-keywords", {
        ...
        body: JSON.stringify({ firstName: firstName.trim(), lastName: lastName.trim(), company: company.trim() || undefined, country, description: description.trim(), keywordsCap }),
      });
      const data = await res.json();
      if (!res.ok || data.error) { setError(data.error ?? "Failed to generate keywords."); return; }
      setEditableKeywords(data.keywords as string[]);
  ...
  const handleRunScan = async () => {
    if (!editableKeywords.length) { setError("Add at least one keyword."); return; }
    setError("");
    setResult(null);
    setScore(0);
    ...
    try {
      const res = await fetch("/api/generate-lead", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ firstName: firstName.trim(), lastName: lastName.trim(), company: company.trim() || undefined, country, description: description.trim(), keywords: editableKeywords, resultsCap }),
      });
      const data = await res.json();
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: "52rem", margin: "0 auto" }}>
        ...
        {/* ── Form card ── */}
        <div className="glass glow-border" style={{ borderRadius: "0.875rem", padding: "clamp(1.25rem, 4vw, 2rem)", marginBottom: "2rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div className="lead-name-grid">
              <div>
                ...
                <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="e.g. John" required style={inputStyle} />
              </div>
              <div>
                <label style={labelStyle}>Last Name *</label>
                <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="e.g. Smith" required style={inputStyle} />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} placeholder="e.g. Acme Corp (optional)" style={inputStyle} />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                required
                style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6, minHeight: "6rem" }}
              />
            </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
                {KEYWORDS_CAP_OPTIONS.map((n) => (
                  <button key={n} type="button" onClick={() => setKeywordsCap(n)}
                    style={{ padding: "0.4rem 1rem", borderRadius: "0.5rem", fontSize: "0.875rem", fontWeight: 500, cursor: "pointer", transition: "all 0.15s", border: "1.5px solid", borderColor: keywordsCap === n ? "#4479DA" : "var(--color-border, #e2e8f0)", backgroundColor: keywordsCap === n ? "#eef3ff" : "#ffffff", color: keywordsCap === n ? "#4479DA" : "var(--color-muted, #64748b)" }}>
                    {n}
                  </button>
                ))}
              </div>
              <p style={{ margin: "0.375rem 0 0", fontSize: "0.75rem", color: "#94a3b8" }}>
                Number of keywords Claude generates from the description
              </p>
            </div>
            ...
            <button type="button" disabled={keywordsLoading} onClick={handleGenerateKeywords} className="glow-button"
              style={{ width: "100%", padding: "0.75rem", fontWeight: 700, borderRadius: "0.625rem", opacity: keywordsLoading ? 0.8 : 1 }}>
              {keywordsLoading ? <><Spinner />Generating Keywords…</> : "Generate Keywords"}
            </button>
            ...
            {keywordsReady && (
              <>
                ...
                <button type="button" disabled={loading || editableKeywords.length === 0} onClick={handleRunScan} className="glow-button"
                  style={{ width: "100%", padding: "0.75rem", fontWeight: 700, borderRadius: "0.625rem", opacity: loading || editableKeywords.length === 0 ? 0.8 : 1 }}>
            ...
            <div className="animate-fade-up" style={{ width: "100%", textAlign: "center" }}>
              ...
              {usedKeywords.length > 0 && (
                <div style={{ marginTop: "1.25rem", textAlign: "left" }}>
                  <p style={{ fontSize: "0.6875rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#94a3b8", marginBottom: "0.5rem" }}>Keywords Used</p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                    {usedKeywords.map((kw) => (
                      <span key={kw} style={{ padding: "0.25rem 0.75rem", borderRadius: "0.5rem", backgroundColor: "rgba(68,121,218,0.08)", border: "1px solid rgba(68,121,218,0.2)", color: "#4479DA", fontSize: "0.8125rem", fontWeight: 500 }}>
                        {kw}
                      </span>
                    ))}
                  </div>
              ...
              {result.summary && (
                <div style={{ marginTop: "1.25rem", borderRadius: "0.75rem", border: "1px solid #e2e8f0", backgroundColor: "#f8fafc", textAlign: "left", overflow: "hidden" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1rem", borderBottom: "1px solid #e2e8f0", backgroundColor: "#f1f5f9" }}>
                    <span style={{ fontSize: "0.625rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#64748b" }}>
